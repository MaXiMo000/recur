...
# /srv, not /app: the Python package is called `app`, and /app/app is a
# needless confusion for anyone reading a stack trace.
WORKDIR /srv
...
RUN useradd --create-home --uid 10001 recur && chown -R recur:recur /srv
USER recur
...
CMD ["sh", "-c", "python -c 'from app import config; config.check()' && alembic upgrade head && exec uvicorn app.api:app --host 0.0.0.0 --port ${PORT:-8000} --proxy-headers --forwarded-allow-ips='*'"]
