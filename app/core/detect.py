...
changed. Signal processing, not prompting -- no model is involved anywhere here.
    ...
    python -m app.core.detect             find subscriptions, print what was found
    python -m app.core.detect --upcoming  what hits the card next
    python -m app.core.detect --increases price changes, newest first
...
def report(conn) -> None:
    ...
    if n_queue:
        print(f"\n  ⚠ {n_queue} descriptors ({n_txn} charges) are still in the review"
              ...
              f"  ->  python -m app.core.resolve --review")
