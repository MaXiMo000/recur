...
"""Tiers 1-2 of the merchant resolution ladder, plus the human review queue.
    ...
    python -m app.core.resolve            resolve everything unresolved, print ladder stats
    python -m app.core.resolve --review   work the pending queue
    python -m app.core.resolve --groups   show what got grouped, to eyeball correctness
...
def print_stats(conn, stats: Counter) -> None:
    ...
    if not all_:
        print("\nno transactions loaded  ->  python -m app.core.ingest sample.csv --account demo")
        return
    ...
    print(f"{n_queue} awaiting human review"
          + ("  ->  python -m app.core.resolve --review" if n_queue else ""))
